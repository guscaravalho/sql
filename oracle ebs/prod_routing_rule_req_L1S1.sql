WITH
    flex_values AS
        (SELECT 
            ...
            ,(CASE 
                WHEN SUBSTR(fv.flex_value,1,3) = 'XXX' THEN 'All Funds'
                ELSE SUBSTR(fv.flex_value,1,3) END)
                                        AS fund
            ,fv.flex_value_meaning      AS cost_center
            ...
            ,fv.attribute1
            ,fv.attribute2
            ,TO_CHAR(fv.attribute3, 'fm999G999G999G999')    AS dollar_limit
            ,fv.attribute4              AS control_type
            ,fv.attribute5
            ,fv.attribute6
        ...
        WHERE 
            fvs.flex_value_set_name = 'ACG_PR_APPROVAL_MATRIX_DEPT_VS')
...
SELECT
    fv.flex_value_id
    ,'Req Workflow L1S1 - By Author'       AS workflow_name
    ,author.department
    ,fv.fund
    ,fv.cost_center
    ,(CASE
        WHEN author.employee_name IS NULL THEN '...vacant...'
            ELSE author.employee_name END)  AS author
    ,(CASE
        WHEN approver.employee_name IS NULL THEN '...vacant...'
            ELSE approver.employee_name END)
                                            AS approver
    ,p.position_name_concat                 AS author_position
    ,p2.position_name_concat                AS approver_position
    ,fv.dollar_limit
    ,fv.rule_status
    ,fv.rule_validity
    ,fv.valid_start_date
...
LEFT OUTER JOIN POSITIONS p
    ON fv.author_position_name = p.position_name_concat
...
LEFT OUTER JOIN EMPLOYEES author
    ON a.assignment_id = author.assignment_id

LEFT OUTER JOIN POSITIONS p2
    ON fv.approver_position_name = p2.position_name_concat

LEFT OUTER JOIN ASSIGNMENTS a2
    ON p2.position_id = a2.position_id
    AND p2.effective_end_date BETWEEN a2.effective_start_date AND a2.effective_end_date

LEFT OUTER JOIN EMPLOYEES approver
    ON a2.assignment_id = approver.assignment_id
