    ...
    journals AS
        (SELECT
            ...
            ,(CASE
                ...
                WHEN SUBSTR(gll.period_name,1,3) = 'MAY' THEN '11 MAY.'
                WHEN SUBSTR(gll.period_name,1,3) = 'JUN' THEN '12 JUN.'
                WHEN SUBSTR(gll.period_name,1,3) = 'ADJ' THEN '12 JUN.' END)
                                                        AS fiscal_month
            ,(CASE 
...
--AP_PMT_DIST                 (payments)
    ...
    ,purchase_orders AS
        (SELECT
            pod.po_distribution_id                          AS purchasing_subledger_dist_id
            ,poh.vendor_id
            ,poh.segment1                                   AS po_number
            ,poh.attribute1                                 AS purchasing_authority_number
            ,poh.attribute4                                 AS quick_quote_number
    ...
    ,requisitions AS
        (SELECT
            DISTINCT prd.distribution_id                    AS requisitions_subledger_dist_id
            ,prl.vendor_id  
...
SELECT
    ...
    ,inv.payables_subledger_dist_id             AS invoice_distribution_id
    ...
    ,j.je_type                                  AS je_type
    ,j.je_category_name                
    ,j.encumbrance_type                         AS encumbrance_type
    ,(CASE
        WHEN COALESCE(inv.po_number, po.po_number, req.po_number) IS NOT NULL
        THEN 'PO '||COALESCE(inv.po_number, po.po_number, req.po_number) END)
                                                AS po_number
    ,(CASE 
        WHEN req.req_number IS NOT NULL THEN 'REQ '||req.req_number END)
                                                AS req_number
    ,COALESCE(inv.purchasing_authority, po.purchasing_authority, req.purchasing_authority)
                                                AS purchasing_authority
    ,COALESCE(sinv.supplier_name, spo.supplier_name, sreq.supplier_name)
                                                AS supplier_name
    ,inv.invoice_number                         AS invoice_number
    ,j.debit                    			    AS je_debit
    ,j.credit                   			    AS je_credit
    ,j.amount                                   AS je_amount
    ,(CASE
        WHEN nal.natural_account_level_1 = '3 Revenue' AND j.je_source = 'Payables' 
            AND j.je_type = '4 Actual' THEN inv.distribution_amount * -1
        WHEN nal.natural_account_level_1 = '4 Expenditure' AND j.je_source = 'Payables' 
            AND j.je_type = '4 Actual' THEN inv.distribution_amount 
        WHEN nal.natural_account_level_1 = '3 Revenue' THEN j.amount * -1
        ELSE j.amount END)                 
                                                AS dist_amount
    ,(CASE
        WHEN nal.natural_account_level_1 = '3 Revenue' AND j.je_source = 'Payables' 
        ...
        WHEN nal.natural_account_level_1 = '4 Expenditure' AND j.je_source = 'Payables' 
            AND j.je_type = '4 Actual' THEN inv.distribution_amount 
        WHEN nal.natural_account_level_1 = '3 Revenue' 
            AND j.je_type IN ('2 Pre-Encumbrance','3 Encumbrance','4 Actual')
            OR nal.natural_account_level_1 = '4 Expenditure' AND j.je_type = '1 Budget' 
        THEN j.amount * -1
        ELSE j.amount END)     
                                                AS dist_amount_pivotable
    ,gla.department                             AS department
    ,nal.natural_account_level_1                AS account_class
    ,nal.natural_account_level_1_5              AS summary_account_type
    ...
    ,j.je_header_name                           AS je_header_name
    ,j.je_header_description                    AS je_header_description
    ,j.je_line_description                      AS je_line_description
    ,j.je_batch_name
    ,j.je_batch_description
    ,j.je_status                                AS je_status
    ,j.set_of_books                             AS set_of_books
    ,j.budget_name                              as budget_name    
    ,j.reference_5     
    ,j.ae_header_id 
    ,j.gl_sl_link_table      
    ,inv.invoice_id 
...
LEFT OUTER JOIN PURCHASE_ORDERS po
    ON ae.subledger_dist_id = po.purchasing_subledger_dist_id
    AND ae.source_distribution_type = 'PO_DISTRIBUTIONS_ALL'

LEFT OUTER JOIN SUPPLIERS spo
    ON po.vendor_id = spo.vendor_id
    
LEFT OUTER JOIN REQUISITIONS req
    ON ae.subledger_dist_id = req.requisitions_subledger_dist_id
    AND ae.source_distribution_type = 'PO_REQ_DISTRIBUTIONS_ALL'

LEFT OUTER JOIN SUPPLIERS sreq
    ON req.vendor_id = sreq.vendor_id
...
WHERE
    j.je_type = '4 Actual'
--    AND nal.natural_account_level_1 = '3 Revenue'
    AND gla.fund_code = '101'
    AND gla.department = '03 Management and Finance'
    AND j.je_line_effective_date BETWEEN '01-JUL-2019' AND '31-AUG-2019'
