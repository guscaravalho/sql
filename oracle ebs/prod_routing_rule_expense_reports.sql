...
SELECT 
    fv.flex_value_set_id
    ,'Expense Report Workflow Approvers'        AS workflow_name
    ,fv.flex_value          AS department_or_role
    ,COALESCE(approver.employee_name,'..vacant..')      AS approver_name
...
WHERE
    fv.flex_value_set_name LIKE 'ACG_AME_APPROVER_GROUPS'
    AND SUBSTR(fv.flex_value,1,3) NOT IN ('HR ', 'HRB')
